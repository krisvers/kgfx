...
static void debugFunc(const char* format) {
	int len = ::MultiByteToWideChar(CP_UTF8, 0, format, static_cast<int>(strlen(format)), nullptr, 0);
	std::wstring fmt(len, L'\0');
	::MultiByteToWideChar(CP_UTF8, 0, format, static_cast<int>(strlen(format)), &fmt[0], len);
...
u32 kgfxGetBackends(KGFXbackend* pBackends, u32 count) {
	...
	pBackends[0] = KGFX_BACKEND_D3D12;
	return 1;
}
...
KGFXpipeline D3D12::createPipeline(KGFXpipelinedesc pipelineDesc) {
	...
	if (uniformBufferCount == 0) {
		...
		rootSignatureDesc.Version = D3D_ROOT_SIGNATURE_VERSION_1_0;
		rootSignatureDesc.Desc_1_0.Flags = D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT;
		rootSignatureDesc.Desc_1_0.NumParameters = static_cast<UINT>(parameters.size());
